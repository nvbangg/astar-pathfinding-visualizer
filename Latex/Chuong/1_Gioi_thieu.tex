...
Điểm khác biệt của hệ thống so với các công bố hiện có nằm ở hai tính năng đột phá. Hệ thống trực quan hóa toàn bộ quá trình tìm kiếm với mã màu phân biệt ba trạng thái của đỉnh: open (màu xanh lá), closed (màu xanh ngọc), path (màu vàng). Hệ thống cung cấp số liệu thống kê chi tiết bao gồm thời gian xử lý (ms), số nút đã duyệt (visited nodes), và số phép tính (operations). Các nội dung chi tiết về cài đặt sẽ được trình bày trong Chương 3.
