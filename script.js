...
canvas.addEventListener("mousedown", (e) => {
  ...
  const [x, y] = pos;
  if (x === startNode[0] && y === startNode[1] && !isVisualized) {
    draggedType = "start";
    return;
  }
  if (x === endNode[0] && y === endNode[1] && !isVisualized) {
    draggedType = "end";
    return;
  }
  if (!isVisualized) {
...
canvas.addEventListener("mousemove", (e) => {
  ...
  if (draggedType) {
    ...
    if (!isSE && !gridData[y][x]) {
      if (draggedType === "start") {
        drawCell(...startNode, "empty");
        startNode = [x, y];
        drawCell(x, y, "start");
      } else {
        drawCell(...endNode, "empty");
        endNode = [x, y];
        drawCell(x, y, "end");
      }
    }
...
function randomWalls() {
  clearWalls();
  for (let y = 0; y < ROWS; y++) for (let x = 0; x < COLS; x++) if (Math.random() < WALL_PROB) gridData[y][x] = 1;
  resetStartEnd();
}
...
document.getElementById("file-input").addEventListener("change", (e) => {
  ...
  reader.onload = (ev) => {
    ...
    for (let y = 0; y < Math.min(ROWS, m.length); y++)
      for (let x = 0; x < Math.min(COLS, m[y].length); x++) gridData[y][x] = m[y][x];
...
function getNeighbors([x, y]) {
  ...
  for (const [dx, dy] of [
    ...
    const nx = x + dx,
      ny = y + dy;
    if (nx >= 0 && nx < COLS && ny >= 0 && ny < ROWS && !gridData[ny][nx]) result.push([[nx, ny], 1.0]);
  }
  if (allowDiag) {
    for (const [dx, dy] of [
      [-1, -1],
...
function runAstar(hName = null) {
  ...
  const heap = {
    ...
    push(item) {
      this.data.push(item);
      this.data.sort((a, b) => a.f - b.f || a.h - b.h || a.tb - b.tb);
    },
    pop() {
  ...
  while (heap.data.length) {
    ...
    closedSet.add(ck);
    const isStartEnd =
      (current[0] === start[0] && current[1] === start[1]) || (current[0] === end[0] && current[1] === end[1]);
    ...
    if (current[0] === end[0] && current[1] === end[1]) {
      ...
      return [
        {
          path_cost: gScore[key(end)].toFixed(2),
          visited_nodes: closedSet.size,
          execution_time: elapsed.toFixed(2),
...
function toggleExecution() {
  if (isRunning) {
    if (isStepMode) {
      isStepMode = isPaused = false;
      setStartBtn("Pause", "#FF9800");
    } else {
      isPaused = !isPaused;
      setStartBtn(isPaused ? "Start" : "Pause", isPaused ? "#4CAF50" : "#FF9800");
    }
  } else {
    clearPath();
...
function playback(index) {
  ...
  if (isPaused && !isStepMode) {
    animTimer = setTimeout(() => playback(index), DELAY_PAUSE);
    return;
  }
  if (isStepMode && !hasStepEvent && index > 0) {
    animTimer = setTimeout(() => playback(index), DELAY_PAUSE);
    return;
  }
  hasStepEvent = false;
...
// ! 6. STATS & COMPARISON
function resetStats() {
  ["st-cost", "st-visited", "st-time", "st-ops"].forEach((id) => (document.getElementById(id).textContent = "-"));
}
...
function compareAll() {
  ...
  table.innerHTML =
    ...
    results
      .map(
        (r) =>
          `<tr class="${r.path_cost !== "-" ? "ok" : "err"}">` + keys.map((k) => `<td>${r[k]}</td>`).join("") + "</tr>",
      )
      .join("");
...
document
  ...
  .addEventListener("click", () => document.getElementById("compare-modal").classList.remove("show"));
