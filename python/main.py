...
COLORS = {
    ...
    "button_pause": "#FF9800",
    "button_step": "#607D8B",
    "button_clear_path": "#E53935",
    "button_clear_walls": "#E53935",
    "button_random": "#795548",
    "button_compare": "#3F51B5",
    "button_save_load": "#607D8B",
    "header": "#E0E0E0",
    "row": "#F9FBE7",
    "error": "#FFEBEE",
}
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.canvas = tk.Canvas(
            self, bg=COLORS["background_main"], highlightthickness=0
        )
        ...
        panel = tk.Frame(
            self, width=320, bg=COLORS["background_panel"], padx=15, pady=15
        )
        ...
        # Settings
        tk.Label(
            panel, text="Settings", font=("", 14, "bold"), bg=COLORS["background_panel"]
        ).pack(anchor="w", pady=(0, 4))
        tk.Label(
            panel, text="Heuristic:", font=("", 11), bg=COLORS["background_panel"]
        ).pack(anchor="w")
        self.combobox_heuristic = ttk.Combobox(
            panel,
            state="readonly",
            width=30,
            font=("", 11),
        ...
        self.checkbutton_allow_diagonal = tk.Checkbutton(
            panel,
            text="Allow Diagonal",
            variable=self.allow_diagonal,
            font=("", 11),
            bg=COLORS["background_panel"],
            command=self._update_ui_state,
        )
        self.checkbutton_allow_diagonal.pack(anchor="w")

        self.checkbutton_dont_cross_corners = tk.Checkbutton(
            panel,
            text="Don't Cross Corners",
            variable=self.dont_cross_corners,
            font=("", 11),
            bg=COLORS["background_panel"],
        )
        self.checkbutton_dont_cross_corners.pack(anchor="w", padx=(20, 0))

        self.checkbutton_diagonal_cost_one = tk.Checkbutton(
            panel,
            text="Diagonal Cost = 1",
            variable=self.diagonal_cost_one,
            font=("", 11),
            bg=COLORS["background_panel"],
        )
        ...
        tk.Label(
            panel, text="Speed:", font=("", 11), bg=COLORS["background_panel"]
        ...
        tk.Label(
            panel, text="Controls", font=("", 14, "bold"), bg=COLORS["background_panel"]
        ).pack(anchor="w", pady=(0, 6))

        button_frame = tk.Frame(panel, bg=COLORS["background_panel"])
        button_frame.pack(fill="x")

        self.button_start_pause = tk.Button(
            button_frame,
            text="Start",
            width=14,
        ...
        tk.Button(
            ...
            text="Clear Path",
            width=14,
            bg=COLORS["button_clear_path"],
            fg=COLORS["button_foreground"],
            font=("", 10, "bold"),
            relief="flat",
            pady=4,
            command=self._clear_search_path,
        ...
        tk.Button(
            ...
            text="Clear Walls",
            width=14,
            bg=COLORS["button_clear_walls"],
            fg=COLORS["button_foreground"],
            font=("", 10, "bold"),
            relief="flat",
            pady=4,
            command=self._clear_all_walls,
        ...
        tk.Button(
            ...
            text="Random Walls",
            width=14,
            bg=COLORS["button_random"],
            fg=COLORS["button_foreground"],
            font=("", 10, "bold"),
            relief="flat",
            pady=4,
            command=self._generate_random_walls,
        ...
        tk.Button(
            ...
            relief="flat",
            pady=4,
            command=self._generate_random_maze,
        ...
        tk.Button(
            ...
            text="Compare All",
            width=30,
            bg=COLORS["button_compare"],
            fg=COLORS["button_foreground"],
            font=("", 10, "bold"),
            relief="flat",
            pady=4,
            command=self._compare_all_heuristics,
        ).grid(row=4, column=0, columnspan=2, padx=3, pady=3)
    ...
    def _handle_mouse_press(self, event):
        ...
        if position == self.start_node and not self.is_visualized:
            ...
            and position != self.end_node
            and not self.is_visualized
        ):
            x, y = position
        ...
        if self.dragged_node_type:
            ...
            if (
                ...
                self._update_cell_color(
                    *position,
                    "start" if self.dragged_node_type == "start_node" else "end",
                )
        elif self.draw_mode:
            self._paint_wall(position)
    ...
    def _generate_random_maze(self):
        ...
        while stack:
            ...
            for dy, dx in [(-2, 0), (2, 0), (0, -2), (0, 2)]:
                ...
                if (
                    1 < nx < GRID_COLUMNS - 2
                    and 1 < ny < GRID_ROWS - 2
                    and self.grid_data[ny][nx]
                ):
    ...
    def _toggle_execution(self):
        if self.is_running:
            if self.is_step_mode:
                ...
                self.button_start_pause.config(
                    text="Start" if self.is_paused else "Pause",
                    bg=COLORS["button_start"]
                    if self.is_paused
                    else COLORS["button_pause"],
                )
        else:
            self._clear_search_path()
    ...
    def _compare_all_heuristics(self):
        ...
        for row_index, result in enumerate(results):
            background_color = (
                COLORS["row"] if result["path_cost"] != "-" else COLORS["error"]
            )
